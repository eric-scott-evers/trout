...
start() -> 
  ...
  io:fwrite(" ~s ~w ~n",[" max(dom)   ", set_tool:max(dom) ]),   
...
% Generate series of four subsets with start and end given
%   consider: [a sub b sub c sub d] 
%   given a and d, find some b and c to satisfy subsets

sub( integer, real    )  -> true;
...
% We only need one solution to prove is_subset true, so we can 
